...
parser = argparse.ArgumentParser(
    description='Reconhecimento de faces com movimento de olhos e fala.')
...
parser.add_argument('--facebook', action='store_true',
                    help='Habilita a interação com likes na página')
parser.add_argument('--geek', action='store_true',
                    help='Habilita as frases especiais da geek city')
...
# Velocidade máxima de rotação em graus / segundo
...
def desenharResultado(image, faces, anguloServos):
    ...
    for angulo in anguloServos:
        ...
        cv2.line(image, (xImagem, 0),
                 (xImagem, image.shape[0]), (255, 0, 255), 2)
        j += 1
    ...
    for (x, y, w, h) in faces:
        if (x, y, w, h) != (0, 0, 0, 0):
            ...
            (x2, y2) = (
                int((x+w) * fatorAmpliacao[0]), int((y+h) * fatorAmpliacao[1]))
    ...
    cv2.putText(image, text, textPos, font, fontSize,
...
def seguirFace(servoIndex, face, image):
    ...
    posicaoRelativaAtualServos[servoIndex] = posicaoRelativaAtualServos[servoIndex] * (
        (ordemMedia-1)/ordemMedia) + (posicaoRelativaFoto) / ordemMedia

    minAngulo = limitesAngulo[servoIndex][0]
    maxAngulo = limitesAngulo[servoIndex][1]

    angulo = limitarValor(maxAngulo - (maxAngulo - minAngulo) *
                          posicaoRelativaAtualServos[servoIndex], limitesAngulo[servoIndex])
...
def anguloParaPosicaoRelativa(angulo, servoIndex):
    ...
    minAngulo = limitesAngulo[servoIndex][0]
    maxAngulo = limitesAngulo[servoIndex][1]
    amplitudeServo = limitesAngulo[servoIndex][1] - \
        limitesAngulo[servoIndex][0]
...
def moverBaseadoNasFaces(faces, image):
    ...
    facesParaIgnorar = []
    def areaFace(face): return face[2]*face[3]
    ...
    if numeroFaces == 0:
        ...
        if numeroFaces == 1:
            ...
            def xFace(face): return face[0]
            facesDirParaEsq = np.asarray(
                sorted(faces, key=xFace, reverse=True))

            def segundaFaceValida(faces): return faces[1][0] != 0
...
''' INICIO '''
def main():
    openCVRecognizer = OpenCVRecognizer(cameraIndex=cameraIndex, resolucaoCaptura=resolucaoCaptura,
                                        resolucaoDeteccao=resolucaoDeteccao, cascadeClassifierPath=cascadeClassifierPath, converterCinza=converterCinza)
    global som
    ...
    while time.time() - start < limiteTempo and openCVRecognizer.isOpened():
        ...
        i = i + 1
        current = time.time()
        print('Processed {0} frames in {1:.2f}s. Avg {2:.2f}fps\r'.format(
            i, (current-start), i/(current-start), end='\r'))
